...
public class AddImg {
    ...
    public void AddUpdateImg(UserInfo user, MultipartFile file) {
        try {
            ...
            user.setOriginalPicture(filePath+"\\"+newFileName);
    ...
    public void AddRecordImg(DoorOpenRecord doorOpenRecord,MultipartFile newFile) {
        try {
            ...
            //封装上传文件位置的全路径  把你要保存的文件的路径和图片的新名字连到一起，得到这个文件位置的全路径
            File newTargetFile = new File(newFilePath, newNewFileName);
            ...
            doorOpenRecord.setContrastingPicture(newFilePath+"\\"+newNewFileName);
        } catch (Exception e) {
